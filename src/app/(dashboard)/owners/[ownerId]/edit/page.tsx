...
export default function EditOwnerPage() {
  ...
  const updateTenant = useUpdateTenant(params.ownerId);

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <Link className="mb-3 inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-primary dark:text-slate-300" href={`/owners/${params.ownerId}`}>
      ...
      {tenant.data ? (
        <TenantForm
          mode="edit"
          formId="owner-edit-form"
          cancelHref={`/owners/${params.ownerId}`}
          submitLabel="Lưu thay đổi"
          isSubmitting={updateTenant.isPending}
          initialValues={tenant.data}
          onSubmit={(values) => updateTenant.mutate(values as UpdateTenantDto, { onSuccess: () => router.push(`/owners/${params.ownerId}`) })}
        />
      ) : null}
    </div>
  );
